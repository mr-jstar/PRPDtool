...
public class DynamicPRPDHistogram {
    ...
    public void addPulses(double[][] pulses) {
        ...
        for (double[] p : pulses) {
            ...
            int xb = phaseBin(phase);
            int yb = ampBin(amp);

            if (xb < 0 || xb >= binsPhase) continue;
            if (yb < 0 || yb >= binsAmp) continue;
    ...
    private int phaseBin(double phase) {
        ...
        return (int)Math.floor(phase / 360.0 * binsPhase);
    }

    private int ampBin(double amp) {
        return (int)Math.floor((amp - ampMin) / (ampMax - ampMin) * binsAmp);
    }
